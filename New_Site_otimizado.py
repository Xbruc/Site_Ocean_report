...
with aba1: ######################  ABA para Report 
    ...
    @st.cache_data
    def carregar_dados(caminho_relativo, sheet_name=0, data_coluna=None):
        """
        ...
        try:
            ...
            data = pd.read_excel(url, sheet_name=sheet_name, engine="openpyxl")
            
            # Caso seja um dicionário, converte para DataFrame
            if isinstance(data, dict):
                st.warning(f"O arquivo '{caminho_relativo}' contém múltiplas planilhas. Usando a primeira.")
                data = next(iter(data.values()))  # Obtém o primeiro DataFrame
            
            # Verifica e converte a coluna de data, se especificada
            if data_coluna:
                if data_coluna in data.columns:
                    data[data_coluna] = pd.to_datetime(data[data_coluna], errors='coerce')
                else:
    ...
    # Dados do arquivo 'Parametros_fisico_quimicos.xlsx' com diferentes abas
    dados_p_fisico_quimicos = carregar_dados('Parametros_fisico_quimicos.xlsx', sheet_name='P_fisico_quimicos')
    dados_material_organico = carregar_dados('Parametros_fisico_quimicos.xlsx', sheet_name='Material_organico')
                    ...
                    if dados_batimetria.empty:
                        ...
                        if not colunas_necessarias.issubset(dados_batimetria.columns):
                            ...
                            fig = px.scatter_mapbox(
                                ...
                                size_max=5,
                                zoom=12,
                                animation_frame="Data" if "year_month" in dados_batimetria.columns else None,
